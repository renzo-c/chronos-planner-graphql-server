...
var _default = {
  ...
  Mutation: {
    createSchedule: (parent, args, {
      db
    }, info) => {
      return db.models.schedule.create(args);
    },
    updateSchedule: (parent, args, {
      db
    }, info) => db.models.schedule.update(args, {
      where: {
        id: args.id
      },
      returning: true,
    ...
    deleteSchedule: (parent, args, {
      db
    }, info) => db.models.schedule.findByPk(args.id).then(schedule => {
    ...
    addEmployeeToSchedule: (parent, {
      ...
      }).then(result => result.addEmployee(employeeUser)).then(() => {
        return db.models.schedule.findOne({
          where: {
            id: scheduleId
          }
    ...
    removeEmployeeToSchedule: (parent, {
      scheduleId,
      employeeUser
    }, {
      db
      ...
      .then(result => result.removeEmployee(employeeUser)).then(() => {
        return db.models.schedule.findOne({
          where: {
            id: scheduleId
          }
